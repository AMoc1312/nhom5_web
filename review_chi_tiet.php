...
<style type="text/css"> #cmtTo, #mocmt {
display:none;} 
}</style>
<?php
}
...
<div id="tbXoa" class="messXoa" style="z-index: 1035;">
   <p style="padding: 15px;margin: 0px;">Bạn có chắc muốn xóa bình luận này không?</p>
   <div style="margin-left: auto; margin-right: auto;text-align: center;background-color: white;">
   
    <button onclick="xoaBinhLuan()" type="submit">Có</button>
 <!--  <a href="thuc_hien_xoa_binh_luan.php?id=<?php echo $id_cmt_xoa;?>"> <button type="submit">Có</button></a>-->
   <button onclick="tatMessTb()">Hủy</button>
    ...
    ;?>
        <div class="col-md-12 col-lg-8">
          ...
          <!-- ĐIỀN TÊN NGƯỜI DÙNG-->
           ...
           <a href="blog_trang_ca_nhan.php?id=<?php echo $row["id_reviewer"];?>"><h3 class="title-single" style="font-size: 30px;">@<?php echo $row["ten_dang_nhap"];?></h3></a> 
          </div>
        </div>
      ...
      <div class="row">
         <div class="col-sm-12">
          ...
          <div class="box-comments" style="max-height: 500px; overflow-y: auto;">
            ...
            <div id="cmtTo" style=" width: 100%;height:auto; border-width: 0px;margin-top: 10px;margin-bottom: 50px;padding-right: 20px;position: -webkit-sticky;position: sticky;top:0px;background-color: white;">
            <form method="POST" action="them_binh_luan_thuc_hien.php" enctype="multipart/form-data">
                  <textarea name="txtBinhLuan1" placeholder="Nhập bình luận..." required  style="padding: 15px;width: 100%;border-style: solid;border-width: 0.1px; border-radius: 20px;clear: both;"></textarea>
                  <button type="submit" style="float:right;z-index: -1;font-size:14px; padding: 5px 15px;background-color: #ccb7b0;color: black;" class="ed"  >Gửi</button>
                  <input type="hidden" name="txtIDrv" value="<?php echo $row["id_review"];?>">
                  </form></div>
                  <div id="messCmt"><p style="font-size: 18px;">Hãy đăng nhập để nhập bình luận</p></div>
                  <!-- đầu commnent to -->
                  <script >
        ...
        $sql1 = "
          SELECT * FROM `tbl_binh_luan` INNER JOIN tbl_user_reviewer on tbl_binh_luan.`id_reviewer`=tbl_user_reviewer.`id_reviewer` WHERE `id_review` = '".$id_rv."' ORDER BY `id_binh_luan` DESC
        ";
      ...
      ;?>
        <!-- ĐOẠN DỮ LIỆU LẶP ĐI LẶP LẠI -->
              <li style="padding-bottom: 5px;"> 
                ...
                <div class="comment-avatar" style="height: 100px;">
                ...
                <div class="comment-details">
                  ...
                  <span style="font-size: 14px;"><?php echo date("d M Y H:i",strtotime($row1["ngay_them"]));?></span>
                  <?php
                  $id_cmt=$row1["id_binh_luan"];
                  $ma_user = (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : '');
                          if ($ma_user==$row1["id_reviewer"]) {
                           
                           ?> 
                           <div  style="float: right;padding-right: 20px;">
                    <script >
                  function batTbxoa() {
                  var x = document.getElementById("tbXoa");
                           x.style.display = "block";
                        }
                  ...
                  var x = document.getElementById("tbXoa");
                        ...
                        {
                          window.location="thuc_hien_xoa_binh_luan.php?id=<?php echo $row["id_review"];?>&id_cmt=<?php echo $row1["id_binh_luan"];?>"

                          $GLOBALS['id_cmt']=$row1["id_binh_luan"];
                        }
                  </script>
                <a href="sua_binh_luan.php?id=<?php echo $row["id_review"];?>&id_cmt=<?php echo $row1["id_binh_luan"];?>"><button style="float: left;color: black;" class="ed"  >Edit</button></a>
                <button onclick="batTbxoa()" style="float: left;color: black;" class="ed" ><img style="width:16px; " src="img/delete.cmt.png"></button>
                  ...
                  <form method="POST" action="sua_binh_luan_thuc_hien.php" enctype="multipart/form-data">
                  <textarea name="txtBinhLuan" placeholder="Nhập bình luận..." required value=""  style="padding: 15px;width: 100%;height:60px;border-style: solid;border-width: 0.1px; border-radius: 20px;clear: both;"></textarea>
                   ...
                   <button type="submit" style="float:right;z-index: -1;font-size:14px; padding: 5px 15px;background-color: #ccb7b0;color: black;" class="ed"  >Gửi</button>
                  <input type="hidden" name="txtIDrv" value="<?php echo $id_rv;?>">
                  </form>
